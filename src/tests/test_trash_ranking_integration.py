...
class TestTrashScraperIntegration:
    ...
    def test_trash_scoring_disabled_by_default(self, sample_movie):
        ...
        _accumulate_ranked_torrents(sample_movie, results, torrents, processed, manual=False)
        ...
        _accumulate_ranked_torrents(sample_movie, results, torrents_trash, processed_trash, manual=False)
...
class TestTrashRankingApiEndpoints:
    ...
    async def test_evaluate_trash_custom_formats_endpoint(self):
        ...
        async with AsyncClient(
            ...
            resp = await client.post("/api/v1/ranking/custom-formats/evaluate", json=payload)
            assert resp.status_code == 200
            data = resp.json()
