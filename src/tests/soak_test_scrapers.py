...
def main():
    ...
    for pass_idx in range(1, TOTAL_PASSES + 1):
        ...
        for i, target in enumerate(targets, start=1):
            print(
                f"[{pass_idx}.{i:02d}] Scraping [{target['category']}] {target['description']} (ID: {target['item_id']})...",
                end=" ",
                flush=True,
            )
            ...
            if res["error"]:
                print(
                    f"FAILED (Status {res['status_code']}, {res['elapsed_ms']:.1f}ms): {res['error']}"
                )
            else:
                funnel = res["funnel"]
                ...
                print(
                    f"OK (200, {res['elapsed_ms']:.1f}ms) -> {res['streams_count']} streams (Funnel: total={total}, ranked={ranked})"
                )
    ...
    successful_requests = sum(
        1 for r in all_results if r["status_code"] == 200 and not r["error"]
    )
    failed_requests = total_requests - successful_requests
    avg_latency = (
        sum(r["elapsed_ms"] for r in all_results) / total_requests
        if total_requests
        else 0
    )
    ...
    print(
        f"Successful (HTTP 200)     : {successful_requests} ({successful_requests / total_requests * 100:.1f}%)"
    )
    ...
    for cat, stats in categories.items():
        ...
        print(
            f"  - {cat:<35}: {stats['success']}/{stats['count']} OK | Avg Latency: {avg_l:7.1f}ms | Streams: {stats['streams']}"
        )
    ...
    p1_latency = (
        sum(r["elapsed_ms"] for r in pass1_results) / len(pass1_results)
        if pass1_results
        else 0
    )
    p2_latency = (
        sum(r["elapsed_ms"] for r in pass2_results) / len(pass2_results)
        if pass2_results
        else 0
    )
    ...
    print(
        f"  Pass 1: {p1_success}/{len(pass1_results)} successful | Avg Latency: {p1_latency:.1f}ms"
    )
    print(
        f"  Pass 2: {p2_success}/{len(pass2_results)} successful | Avg Latency: {p2_latency:.1f}ms"
    )

    # Final Assertions
    assert failed_requests == 0, (
        f"Soak test encountered {failed_requests} failed requests!"
    )
