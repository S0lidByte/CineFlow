    ...
    def test_all_states_represented_in_allowed_transitions(self):
        for state in States:
            assert state in ALLOWED_TRANSITIONS, (
                f"Missing {state} in ALLOWED_TRANSITIONS"
            )
...
class TestProviderErrors:
    def test_provider_error_base(self):
        err = ProviderError(
            "connection reset", provider_name="RealDebrid", is_transient=True
        )
    ...
    def test_provider_rate_limit_error(self):
        err = ProviderRateLimitError(
            "Too Many Requests", provider_name="Zilean", retry_after_seconds=30.0
        )
        assert err.is_transient is True
    ...
    def test_provider_network_and_unavailable_errors(self):
        net_err = ProviderNetworkError(
            "DNS resolution failed", provider_name="Prowlarr"
        )
        assert net_err.is_transient is True

        unavail_err = ProviderUnavailableError(
            "503 Service Unavailable", provider_name="AllDebrid"
        )
        assert unavail_err.is_transient is True
        assert unavail_err.status_code == 503

    def test_provider_quota_exceeded_error(self):
        quota_err = ProviderQuotaExceededError(
            "Account points expired", provider_name="Premiumize"
        )
