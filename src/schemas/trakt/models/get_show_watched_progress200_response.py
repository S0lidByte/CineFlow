...
class GetShowWatchedProgress200Response(BaseModel):
    """
    GetShowWatchedProgress200Response
    """  # noqa: E501

    aired: Optional[Union[StrictFloat, StrictInt]] = None
    completed: Optional[Union[StrictFloat, StrictInt]] = None
    last_watched_at: Optional[StrictStr] = None
    reset_at: Optional[Any] = None
    seasons: Optional[List[GetShowWatchedProgress200ResponseSeasonsInner]] = None
    hidden_seasons: Optional[List[GetTrendingComments200ResponseInnerSeason]] = None
    next_episode: Optional[CheckIntoAnItem201ResponseEpisode] = None
    last_episode: Optional[CheckIntoAnItem201ResponseEpisode] = None
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of GetShowWatchedProgress200Response from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in hidden_seasons (list)
        _items = []
        if self.hidden_seasons:
            for _item_hidden_seasons in self.hidden_seasons:
                if _item_hidden_seasons:
                    _items.append(_item_hidden_seasons.to_dict())
            _dict["hidden_seasons"] = _items
        # override the default output from pydantic by calling `to_dict()` of next_episode
        if self.next_episode:
            _dict["next_episode"] = self.next_episode.to_dict()
        # override the default output from pydantic by calling `to_dict()` of last_episode
        if self.last_episode:
            _dict["last_episode"] = self.last_episode.to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of GetShowWatchedProgress200Response from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate(
            {
                ...
                "last_watched_at": obj.get("last_watched_at"),
                "reset_at": obj.get("reset_at"),
                "seasons": (
                    [
                        GetShowWatchedProgress200ResponseSeasonsInner.from_dict(_item)
                ...
                "hidden_seasons": (
                    [
                        GetTrendingComments200ResponseInnerSeason.from_dict(_item)
                        for _item in obj["hidden_seasons"]
                    ]
                    if obj.get("hidden_seasons") is not None
                    else None
                ),
                "next_episode": (
                    CheckIntoAnItem201ResponseEpisode.from_dict(obj["next_episode"])
                    if obj.get("next_episode") is not None
                    else None
                ),
                "last_episode": (
                    CheckIntoAnItem201ResponseEpisode.from_dict(obj["last_episode"])
                    if obj.get("last_episode") is not None
                    else None
                ),
            }
        )
